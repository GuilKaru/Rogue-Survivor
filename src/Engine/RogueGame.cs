...
namespace RogueSurvivor.Engine
{
    class RogueGame : GameState
    {
...
#else
                        // in release, instead of "crashing" the game by throwing an exception force the AI to do
        ...
        void NextMapTurn(Map map, SimFlags sim)
        {
            ...
            //      2.1 Odors decay.
            //      2.2 Actors scents.
            // 3. Regen actors AP & STA
            // 4. Stop tired actors from running.
            // 5. Actor gauges & states :
            //    Hunger, Sleep, Sanity, Leader Trust.
            //      5.1 May kill starved actors.
            //      5.2 Handle sleeping actors.
            //      5.3 Exhausted actors might collapse.
            // 6. Check batteries : lights, trackers.
            // 7. Check explosives.
            // 8. Check fires.
            ...
            if (!isLoDetail)
            {
                ...
                foreach (Actor actor in map.Actors)
                {
                    ...
                    if (light != null)
                    {
                        ...
                        {
                            --light.Batteries;
                            if (light.Batteries <= 0 && IsVisibleToPlayer(actor))
                    ...
                    if (tracker != null)
                    {
                        ...
                        {
                            --tracker.Batteries;
                            if (tracker.Batteries <= 0 && IsVisibleToPlayer(actor))
                ...
                // 8. Check fires.
                // 8.1 Rain has a chance to put out fires.
        ...
        void HandlePlayerAction(Actor player)
        {
            ...
            if (key != Key.None)
            {
                ...
                if (command != PlayerCommand.NONE && (command.IsSpecial() || !TryPlayerInsanity()))
                {
                    switch (command)
                    {
                        case PlayerCommand.QUIT_GAME: // Shift Q
                            HandleQuitGame();
                            break;

                        case PlayerCommand.ABANDON_GAME: // Shift A
                            HandleAbandonGame();
                            break;

                        case PlayerCommand.HELP_MODE: // H
                            PushState<HelpState>();
                            break;

                        case PlayerCommand.HINTS_SCREEN_MODE: // Ctrl H
                            PushState<HintsState>();
                            break;

                        case PlayerCommand.ADVISOR: // Shift H
                            HandleAdvisor();
                            break;

                        case PlayerCommand.OPTIONS_MODE: // Shift O
                            PushState<OptionsState>();
                            break;

                        case PlayerCommand.KEYBINDING_MODE: // Shift K
                            PushState<RedefineKeysState>();
                            break;

                        case PlayerCommand.MESSAGE_LOG: // Shift M
                        ...
                        case PlayerCommand.CITY_INFO: // I
        ...
        public void DoReviveCorpse(Actor actor, Corpse corpse)
        {
            ...
            // spend medikit.
            Item medikit = actor.Inventory.GetSmallestStackByModel(Items.MEDIKIT);
        ...
        {
            if (m_Rules.CanActorShout(m_Player, out string reason))
                DoShout(m_Player, null);
            else
        ...
        void HandlePlayerBreak()
        {
            ...
            waitForDir = dir =>
            {
                ...
                else
                {
                    ...
                    {
                        Exit exitThere = m_Player.Location.Map.GetExitAt(m_Player.Location.Position);
                        if (exitThere == null)
                        ...
                        else
                        {
                            ...
                            {
                                // only if enemy.
                                if (m_Rules.AreEnemies(m_Player, actorTo))
                                {
                                    // check melee rule.
                                    if (m_Rules.CanActorMeleeAttack(m_Player, actorTo, out reason))
                                    {
                                        DoMeleeAttack(m_Player, actorTo);
                                        ClearOverlays();
                                        return false;
                                    }
                                    else
                            ...
                            else
                            {
                                ...
                                {
                                    // check break rule.
                                    if (m_Rules.IsBreakableFor(m_Player, objTo, out reason))
                                    {
                                        DoBreak(m_Player, objTo);
                                        ClearOverlays();
                                        return false;
                                    }
                                    else
                    ...
                    else
                    {
                        ...
                        if (m_Player.Location.Map.IsInBounds(pos))
                        {
                            ...
                            if (mapObj != null)
                            {
                                if (m_Rules.IsBreakableFor(m_Player, mapObj, out string reason))
                                {
                                    DoBreak(m_Player, mapObj);
                                    ClearOverlays();
                                    return false;
                                }
                                else
        ...
        {
            // check autosave before player starts long wait
            CheckAutoSaveTime();
        ...
        string[] DescribeActor(Actor actor)
        {
            ...
            if (!m_Player.Faction.IsEnemyOf(actor.Faction) && m_Rules.AreGroupEnemies(m_Player, actor))
        ...
        string[] DescribeItemTrap(ItemTrap tr)
        {
            ...
            // 3. Stats
            lines.Add(string.Format("Damage  : {0} x{1} = {2}", m.Damage, tr.Quantity, tr.Quantity * m.Damage));
            lines.Add(string.Format("Trigger : {0}% x{1} = {2}%", m.TriggerChance, tr.Quantity, tr.Quantity * m.TriggerChance));
        ...
        string[] DescribeItemEntertainment(ItemEntertainment ent)
        {
            ...
            // player bored?
            if (m_Player != null && ent.IsBoringFor(m_Player))
        ...
        public void DoLeaveMap(Actor actor, Point exitPoint, bool askForConfirmation)
        {
            ...
            if (isPlayer && askForConfirmation)
            {
                ...
                {
                    if (yes)
                        DoLeaveMap(actor, exitPoint, false);
                    else
            ...
            // check autosave before player leaving map
            if (isPlayer)
                CheckAutoSaveTime();
        ...
        public void DoUseExit(Actor actor, Point exitPoint)
        {
            // leave map.
            DoLeaveMap(actor, exitPoint, false);
        }
        ...
        public void DoMeleeAttack(Actor attacker, Actor defender)
        {
            ...
            // clear overlays only if action is visible
            if (isAttVisible || isDefVisible)
                ClearOverlays();
        }
        ...
        // "fast" trade uses new trade mechanic of rating items and trades.
        ...
        void DoUseFoodItem(Actor actor, ItemFood food)
        {
            ...
            {
                ItemTrap emptyCan = new ItemTrap(Items.EMPTY_CAN);
                emptyCan.Activate(actor);
        ...
        public void DoBarricadeDoor(Actor actor, DoorWindow door)
        {
            ...
            ItemBarricadeMaterial it = actor.Inventory.GetSmallestStackByType(typeof(ItemBarricadeMaterial)) as ItemBarricadeMaterial;
        ...
        public void DoPush(Actor actor, MapObject mapObj, Point toPos)
        {
            ...
            // followers help?
            if (actor.CountFollowers > 0)
                DoPushPullFollowersHelp(actor, mapObj, false, ref staCost);
        ...
        public void RedrawPlayScreen()
        {
            ...
            ui.Clear(Color.Black);
            {
                ...
                // desc day fov effect, not if cant know time
        ...
        public void DrawActorSprite(Actor actor, Point screen, Color tint)
        {
            ...
            if (m_Player != null)
            {
                ...
                bool groupEnemies = !m_Player.Faction.IsEnemyOf(actor.Faction) && m_Rules.AreGroupEnemies(m_Player, actor);
                if (imSelfDefence)
            ...
            // or has needed item (not for undead player duh)
            if (m_Player != null)
            {
            ...
            // odor suppressed icon (will overlap with sleep healing but its fine)
            if (actor.OdorSuppressorCounter > 0)
        ...
        public void DrawInventory(Inventory inventory, string title, bool drawSlotsNumbers, int slotsPerLine, int maxSlots, int gx, int gy)
        {
            ...
            foreach (Item it in inventory.Items)
            {
                ...
                {
                    ItemTrap trap = it as ItemTrap;
                    DrawTrapItem(trap, x, y);
                }
                else if (it is ItemEntertainment)
                {
                    if (m_Player != null && ((it as ItemEntertainment).IsBoringFor(m_Player)))
        ...
        public void DrawActorSkillTable(Actor actor, int gx, int gy)
        {
            ...
            foreach (Skill sk in skills)
            {
                ...
                // highlight if active skills are active or not
                switch (sk.ID)
                {
        ...
        void BeforePlayerEnterDistrict(District district)
        {
            ...
            {
                int catchupTo = m_Session.WorldTime.TurnCounter;
                int turnsToCatchup = catchupTo - entryMap.LocalTime.TurnCounter;
        ...
        void StartSimThread()
        {
            if (s_Options.IsSimON && s_Options.SimThread)
            {
                ...
                lock (m_SimStateLock) { m_SimThreadDoRun = true; };
                m_SimThread.Start();
            }
        }

        // StopSimThread is now blocking until the sim thread has actually stopped
        ...
        void CheckSpecialPlayerEventsAfterAction(Actor player)
        {
                    ...
                    case ScriptStage.STAGE_1:  // offered deal, waiting for opened cell.
                            ...
                            !prisoner.IsSleeping)
                        {
                            lock (m_Session) // thread safe
                            {
                                ...
                                // transformation.
                                // - zombify.
                                KillActor(null, prisoner, "transformation", false);
            ...
            foreach (Point p in m_PlayerFOV)
            {
                ...
                // unique npcs lose their invincibility when sighted and highlight them.
                if (other.IsUnique && other.IsInvincible)  // 1st sighting
                {
